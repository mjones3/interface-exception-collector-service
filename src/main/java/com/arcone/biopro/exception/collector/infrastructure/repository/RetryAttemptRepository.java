...
public interface RetryAttemptRepository extends JpaRepository<RetryAttempt, Long> {
     ...
     */
    Optional<RetryAttempt> findTopByInterfaceExceptionOrderByAttemptNumberDesc(InterfaceException interfaceException);
     ...
     */
    @Query("SELECT ra FROM RetryAttempt ra WHERE " +
            "ra.resultErrorDetails IS NOT NULL AND " +
            "LOWER(ra.resultErrorDetails) LIKE LOWER(CONCAT('%', :errorText, '%'))")
    Page<RetryAttempt> findByErrorDetailsContaining(@Param("errorText") String errorText, Pageable pageable);
