...
public class SummaryService {
    ...
    private float calculateAverageResolutionTime(OffsetDateTime startDate, OffsetDateTime endDate) {
        try {
            ...
            List<Object[]> resolutionData = exceptionRepository.getAverageResolutionTime(startDate, endDate);

            if (resolutionData.isEmpty()) {
                return 0.0f;
            }

            // Calculate average from the query result
            Double avgHours = (Double) resolutionData.get(0)[0];
            return avgHours != null ? avgHours.floatValue() : 0.0f;
