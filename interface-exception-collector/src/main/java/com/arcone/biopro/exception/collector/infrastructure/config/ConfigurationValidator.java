...
public class ConfigurationValidator {
    ...
    private void validateServiceConfig(String serviceName, ApplicationProperties.SourceServices.ServiceConfig config, List<String> errors) {
        if (config == null) {
    ...
    private void logConfigurationSummary() {
        ...
        logger.info("    Order Service: {}", maskSensitiveUrl(applicationProperties.sourceServices().order().baseUrl()));
        logger.info("    Collection Service: {}", maskSensitiveUrl(applicationProperties.sourceServices().collection().baseUrl()));
        logger.info("    Distribution Service: {}", maskSensitiveUrl(applicationProperties.sourceServices().distribution().baseUrl()));
    }
        ...
        try {
            ...
            if (userInfo != null) {
                ...
                String maskedUserInfo = userInfo.contains(":") ? 
                    userInfo.substring(0, userInfo.indexOf(":") + 1) + "***" : userInfo;
