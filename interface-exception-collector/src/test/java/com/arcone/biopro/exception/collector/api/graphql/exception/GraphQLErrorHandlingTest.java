 ...
 * Tests proper error classification, message sanitization, and error extensions.
 */
...
class GraphQLErrorHandlingTest {
    ...
    void setUp() {
        ...
        when(dataFetchingEnvironment.getExecutionStepInfo()).thenReturn(mock(graphql.execution.ExecutionStepInfo.class));
        when(dataFetchingEnvironment.getExecutionStepInfo().getPath()).thenReturn(mock(graphql.execution.ResultPath.class));
    ...
    void resolveException_WithMultipleValidationErrors_ShouldReturnMultipleErrors() throws Exception {
        ...
        assertThat(errors).allMatch(error -> 
                error.getErrorType() == GraphQLErrorType.VALIDATION_ERROR
        );
    }
