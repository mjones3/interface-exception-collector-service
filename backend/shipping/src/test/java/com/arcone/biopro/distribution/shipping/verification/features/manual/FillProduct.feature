...
Feature: Manual Test of Fill Products.
    ...
    @DIS-231
        Scenario: Product Code field enabled with Visual Inspection enabled.
    ...
    @DIS-232
        Scenario: Product Code briefly enabled after a previous product has been filled with Visual Inspection disabled.
    ...
    @DIS-283
        Scenario: Verify that no Error messages displayed every time a valid product is scanned when Visual Inspection disabled.
            Given I am filling a product, and the visual inspection is not configured,
            When I scan a valid unit number and a valid product code,
            Then I see the product added to the list,
