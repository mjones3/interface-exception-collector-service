...
@Slf4j
@Profile("prod")
public class OrderRejectedListener extends AbstractKafkaListener {

    private static final String ORDER_REJECTED_SCHEMA = "schema/order-rejected-outbound.json";
    ...
    protected Mono<ReceiverRecord<String, String>> handleMessage(ReceiverRecord<String, String> event) {
        try {
            var message = objectMapper.readValue(event.value(), OrderRejectedEventDTO.class);
            return schemaValidationService.validateSchema(event.value(), ORDER_REJECTED_SCHEMA)
                .then(Mono.defer(() -> orderService
                            .processOrderRejectedEvent(message.payload())))
            ...
            log.error(String.format("Problem deserializing an instance of [%s] " +
                    "with the following json: %s ", OrderRejectedEventDTO.class.getSimpleName(), event), e);
            sendToDlq(event.value(), e.getMessage());
