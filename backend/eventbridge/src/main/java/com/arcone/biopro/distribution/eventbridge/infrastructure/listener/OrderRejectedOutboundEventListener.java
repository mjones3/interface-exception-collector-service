...
public class OrderRejectedOutboundEventListener {
    ...
    public void handleOrderRejectedOutboundEvent(OrderRejectedOutboundEvent event) {
        log.debug("Order Rejected Outbound event trigger Event ID {}", event.getEventId());

        var message = new EventMessage<>("OrderRejectedOutbound","1.0",orderRejectedOutboundMapper.toDto(event.getPayload()));
