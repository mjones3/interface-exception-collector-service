...
import com.arcone.biopro.distribution.receiving.infrastructure.listener.DeviceCreatedListener;
...
class DeviceCreatedListenerTest {
    ...
    public void shouldHandleMessage() throws Exception{
        ...
        Mockito.when(deviceService.createDevice(Mockito.any(DeviceCreatedMessage.class))).thenReturn(Mono.empty());
        ...
        Mockito.when(receiverRecord.value()).thenReturn(TestUtil.resource("events/device-created-event.json").replace("{DEVICE_ID}", "W035625205983"));
        ...
        var listener = new DeviceCreatedListener(consumer,objectMapper,deviceService,producerTemplate,"TEST");

        listener.run(new String[]{""});

        Mockito.verify(deviceService).createDevice(Mockito.any(DeviceCreatedMessage.class));
    }
    ...
    public void shouldNotHandleMessageWhenMessageIsInvalid() throws Exception {
        ...
        var message = Mockito.mock(DeviceCreatedMessage.class);

        Mockito.when(message.getEventId()).thenReturn(UUID.randomUUID().toString());

        Mockito.when(receiverRecord.key()).thenReturn("test");
        Mockito.when(receiverRecord.value()).thenReturn(TestUtil.resource("events/device-created-event.json").replace("\"location\": \"234567891\"", ""));
        Mockito.when(receiverRecord.topic()).thenReturn("DeviceUpdated");
        Mockito.when(receiverRecord.offset()).thenReturn(1L);
