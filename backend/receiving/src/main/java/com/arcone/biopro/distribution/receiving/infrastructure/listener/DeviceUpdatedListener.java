...
@Slf4j
@Profile("prod")
public class DeviceUpdatedListener extends AbstractKafkaListener{
    ...
    public DeviceUpdatedListener(
        @Qualifier(KafkaConfiguration.DEVICE_UPDATED_CONSUMER) ReactiveKafkaConsumerTemplate<String, String> consumer
    ...
    protected Mono<ReceiverRecord<String, String>> handleMessage(ReceiverRecord<String, String> event) {
        try {
            var message = objectMapper.readValue(event.value(), DeviceUpdatedMessage.class);
            return deviceService.updateDevice(message)
            ...
            log.error(String.format("Problem deserializing an instance of [%s] " +
                "with the following json: %s ", DeviceUpdatedMessage.class.getSimpleName(), event), e);
            sendToDlq(event.value(), e.getMessage());
