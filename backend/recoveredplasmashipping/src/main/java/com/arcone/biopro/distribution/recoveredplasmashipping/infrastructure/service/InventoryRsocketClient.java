...
public class InventoryRsocketClient implements InventoryService {
    ...
    @Override
    public Flux<InventoryValidation> validateInventoryBatch(Flux<ValidateInventoryCommand> validateInventoryCommand) {
        return rSocketRequester
            .route("validateInventoryBatch")
            .data(validateInventoryCommand.map(commandMapper::toRequest))
            .retrieveFlux(InventoryValidationResponseDTO.class)
            .map(inventoryMapper::toValidationModel)
            .onErrorResume(error -> {
                    log.error("Error On Validate inventory Batch {}",error.getMessage());
                    return  Flux.error(new InventoryServiceNotAvailableException(RecoveredPlasmaShippingServiceMessages.INVENTORY_SERVICE_NOT_AVAILABLE_ERROR));
                }
            );
    }
