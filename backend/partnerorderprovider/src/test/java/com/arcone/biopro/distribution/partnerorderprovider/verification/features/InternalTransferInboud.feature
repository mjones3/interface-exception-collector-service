...
Feature: Receive internal transfer orders through a third party application
    ...
    Scenario Outline: Receive a Partner Internal Transfer order inbound request
        Given I have a Partner Internal Transfer order with Location Code as "<LocationCode>", Shipment Type as "<ShipmentType>", Ship To Location code as "<shipping Customer Code>", Label Status as "<LabelStatus>" and Quarantine Products as "<QuarantineProducts>".
        ...
        Examples:
            |LocationCode | ShipmentType      | shipping Customer Code | LabelStatus | QuarantineProducts | responseCode | status   |
    ...
    Scenario Outline: Validate Partner Internal Transfer order inbound request
        ...
        Examples:
            |LocationCode | ShipmentType      | shipping Customer Code | LabelStatus      | QuarantineProducts  | responseCode | errorMessage                                                                                              |
           ...
           |  123456789  | INTERNAL_TRANSFER |  <null>                 | LABELED          |   true              |   400        |  $.shippingCustomerCode: null found, but [string] is required                                                        |
