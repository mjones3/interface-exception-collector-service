...
import com.arcone.biopro.distribution.order.infrastructure.listener.OrderCompletedListener;
...
class OrderCompletedListenerTest {
    ...
    private OrderCompletedListener target;

    private ReactiveKafkaProducerTemplate<String, OrderCompletedDTO> producerTemplate;


    @BeforeEach
    public void setUp(){
        producerTemplate = Mockito.mock(ReactiveKafkaProducerTemplate.class);
        target = new OrderCompletedListener(producerTemplate,"TestTopic");
    }
    ...
    public void shouldHandleOrderCompletedEvents(){
        ...
        target.handleOrderCompletedEvent(new OrderCompletedEvent(order));
