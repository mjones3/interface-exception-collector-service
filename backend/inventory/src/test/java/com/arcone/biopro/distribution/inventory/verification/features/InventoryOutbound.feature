...
@api @skipOnPipeline @cleanUpAll @A0A-25
Feature: Inventory Outbound Interface
    Rule: As an inventory service I want to create an inventory update event every time an inventory is created/updated
        Scenario Outline: Produce an inventory updated event every time an inventory is create/update
            ...
            @R20-333
            Examples:
                | Event              | Unit Number   | Product Code | Final Product Code | Is licensed | Shipment type | Reason            | Reason Id | Device Storage | Storage Location        | Location   | Update Type        | Expiration Date | Status Included |
            ...
            @LAB-503
            Examples:
                | Event            | Unit Number   | Product Code | Final Product Code | Is licensed | Abo Rh | Reason | Reason Id | Device Storage | Storage Location | Location | Update Type   | Expiration Date     | Status Included |
                | Product Imported | W777725004010 | E1624V00     | E1624V00           | true        | OP     |        |           |                |                  |          | LABEL_APPLIED | 2011-12-03T09:15:30 |                 |

            @R20-804
            Examples:
                | Event             | Unit Number   | Product Code | Final Product Code | Is licensed | Shipment type | Reason            | Reason Id | Device Storage | Storage Location | Location | Expiration Date | Status Included      | Update Type        |
                | Label Applied     | W777725004051 | E162400      | E1624V00           | true        |               |                   |           |                |                  |          | 02/15/2026      | LABELED              | LABEL_APPLIED      |
                | Apply Quarantine  | W777725004052 | E1624V00     | E1624V00           |             |               | Quarantine Reason | 1         |                |                  |          | 02/15/2026      | LABELED, QUARANTINED | QUARANTINE_APPLIED |
                | Remove Quarantine | W777725004053 | E1624V00     | E1624V00           |             |               | Quarantine Reason | 2         |                |                  |          | 02/15/2026      | LABELED              | QUARANTINE_REMOVED |
