    ...
    void execute_ShouldReturnInventoryOutput_WhenInventoryIsFoundAndQuarantineIsRemoved() {
        ...
        StepVerifier.create(result)
            .consumeNextWith(output -> {
                assertThat(output.inventoryStatus()).isEqualTo(InventoryStatus.AVAILABLE);  // Verificando que as quarentenas foram removidas
            })
            .verifyComplete();

        // Verificando o estado diretamente da instância
        assertThat(inventoryAggregate.getInventory().getQuarantines()).isEmpty();  // Após a remoção, a lista de quarentenas deve estar vazia
