...
public class IrradiationController {
    ...
    private final CompleteBatchUseCase completeBatchUseCase;
    ...
    @MutationMapping
    public Mono<BatchSubmissionResult> completeBatch(@Argument("input") CompleteBatchInput input) {
        return completeBatchUseCase.execute(mapper.toCompleteBatchCommand(input))
                .map(mapper::toGraphQLResult);
    }
}