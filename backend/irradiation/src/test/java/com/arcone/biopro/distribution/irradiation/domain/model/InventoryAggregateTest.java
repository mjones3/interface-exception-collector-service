...
class InventoryAggregateTest {
    ...
    void testIsExpired_ShouldReturnTrue_WhenExpirationDateIsBeforeNow() {
        ...
        assertTrue(inventoryAggregate.isExpired(), "Expected irradiation to be expired");
    }

    @Test
    void testIsExpired_ShouldReturnFalse_WhenExpirationDateIsAfterNow() {
        when(inventoryMock.getExpirationDate()).thenReturn(LocalDateTime.now().plusDays(1));
        assertFalse(inventoryAggregate.isExpired(), "Expected irradiation to not be expired");
    }
    ...
    void testCheckIfIsValidToShip_ShouldAddNotification_WhenInventoryIsExpired() {
        ...
        assertFalse(inventoryAggregate.getNotificationMessages().isEmpty(), "Expected notification messages when irradiation is expired");
    ...
    @Test
    @DisplayName("isAvailable should return true when irradiation status is AVAILABLE")
    ...
    @Test
    @DisplayName("isAvailable should return false when irradiation status is not AVAILABLE")
    ...
    @Test
    @DisplayName("getIsLabeled should return irradiation labeled status")
