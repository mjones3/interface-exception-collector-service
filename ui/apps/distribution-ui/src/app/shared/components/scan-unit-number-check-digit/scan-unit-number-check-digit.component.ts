...
import { RsaValidators, ValidationType } from '@rsa/commons';
...
export class ScanUnitNumberCheckDigitComponent implements OnInit {
  @Input() labelTitle = 'scan-or-enter-unit-number.label';
  ...
  constructor(protected fb: FormBuilder) {
    this.form = this.fb.group({
      ...
      checkDigit: [{ value: null, disabled: true }, [...this.checkDigitValidators]],
    });
  }
  ...
  checkValues(event) {
    ...
    const hasValue = !!unitNumber && (scanner || (!this.disableCheckDigit && !!checkDigit) || this.disableCheckDigit);

    if (hasValue && this.form.valid && this.validate) {
      if (!this.disableCheckDigit) {
        this.controlCheckDigit.setValidators([...this.checkDigitValidators]);
        this.controlCheckDigit.updateValueAndValidity();
      }
  ...
  enabledCheckDigit(event: string) {
    const scanner = this.controlUnitNumber.value ? this.controlUnitNumber.value.startsWith('=') : false;
    ...
    if (!this.disableCheckDigit) {
      if (scanner) {
        ...
        if (this.controlUnitNumber.valid && this.controlUnitNumber.value) {
          this.controlCheckDigit.enable({ emitEvent: false, onlySelf: true });
        } else {
          this.controlCheckDigit.disable({ emitEvent: false, onlySelf: true });
        }
      }
