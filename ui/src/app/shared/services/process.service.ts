...
type processProductVersionResponseType =
    HttpResponse<ProcessProductVersionModel>;
...
export class ProcessService {
    ...
    get processVersionUrl(): string {
        return `/${this._process_version_Url}`;
    }
    ...
    getProcessConfiguration(
        uuid: string
    ): Observable<processProductResponseType> {
        return this.httpClient
            .get<ProcessProductModel>(`${this.processUrl}/${uuid}`, {
                observe: 'response',
            })
     ...
     */
    getProcessProductVersion(
        uuid: string
    ): Observable<processProductVersionResponseType> {
        const params = new HttpParams().set('versionType', 'build_version');

        return this.httpClient
            .get<ProcessProductVersionModel>(
                `${this.processVersionUrl}/${uuid}`,
                {
                    params: params,
                    observe: 'response',
                }
