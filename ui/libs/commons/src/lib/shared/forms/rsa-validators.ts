...
export class RsaValidators {
  ...
  static matchStringFn(matchValue: string): ValidatorFn {
    const validation = (formGroup: FormGroup): ValidationErrors | null => {
      const currentValue = formGroup.value;
      if (currentValue && matchValue && currentValue.toLowerCase() !== matchValue.toLowerCase()) {
  ...
  static matchStringArrFn(inputsArr: any, matchValue: string): ValidatorFn {
    const validation = (formGroup: FormGroup): ValidationErrors | null => {
      ...
      inputsArr.forEach(currentValue => {
        if (currentValue.value && matchValue && currentValue.value !== matchValue) {
          returnValue = true;
        }
      });
  ...
  static matchSelectStringFn(valueTobeMatched: string, matchValue: string): ValidatorFn {
    const validation = (formGroup: FormGroup): ValidationErrors | null => {
      const currentValue = formGroup.value;
      if (currentValue && matchValue && currentValue.selectionKey !== matchValue) {
        return { notMatch: true };
      }
      return null;
    };
  ...
  static timeValidator(dateControl: string, hoursControl: string, minutesControl: string): ValidatorFn {
    const validation = (formGroup: FormGroup): ValidationErrors | null => {
      ...
      if (formGroup.get(hoursControl).valid && formGroup.get(minutesControl).valid) {
        return date &&
          date.diff(todayDate, 'days') === 0 &&
          (hours > todayDate.getHours() || (hours === todayDate.getHours() && minutes > todayDate.getMinutes()))
          ? { invalidTime: true }
          : null;
      } else {
        return null;
  ...
  static hasAtLeastOne(control: AbstractControl): ValidationErrors | null {
    const validation =
      ...
      Object.keys(control.value).some(field => {
        return (
          isNotNullOrUndefined(control.value[field]) &&
          control.value[field] !== '' &&
          (!Array.isArray(control.value[field]) ||
            (Array.isArray(control.value[field]) && control.value[field].length > 0))
        );
      });
