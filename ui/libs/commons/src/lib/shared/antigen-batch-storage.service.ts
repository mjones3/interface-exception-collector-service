...
import { AntigenBatchEntryDto } from '@rsa/commons';
...
export class AntigenBatchStorageService {
  ...
  setCurrentTestingMethod(testingMethod: string) {
    this.storage.setItem(this.TESTING_METHOD, testingMethod);
  }

  getCurrentTestingMethod(): string {
    return this.storage.getItem(this.TESTING_METHOD);
  }
  ...
  getCurrentWorkstations(): { name: string }[] {
    return JSON.parse(this.storage.getItem(this.WORKSTATIONS));
  }

  setCurrentWorkstations(workstations: { name: string }[]) {
