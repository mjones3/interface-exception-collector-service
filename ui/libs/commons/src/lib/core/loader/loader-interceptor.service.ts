...
import { Headers } from '../../shared/types/headers.enum';
...
export const LOADER_BLACKLIST_URLS = new InjectionToken<LoaderBlackListUrls>('Loader Blacklist Urls', {
  providedIn: 'root',
  factory: blacklistFn,
});
...
export class LoaderInterceptorService implements HttpInterceptor {
  ...
  constructor(
    ...
    @Inject(LOADER_BLACKLIST_URLS) private blackListUrls: LoaderBlackListUrls
  ) {}

  public intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    const ignoreLoader = req.headers.get(Headers.XIgnoreLoader) === HeaderValue.True || this.isInBlackList(req.url);
    if (!ignoreLoader) {
  ...
  private isInBlackList(url: string) {
    return this.blackListUrls.urls.filter(item => url.indexOf(item) >= 0).length > 0;
  }
