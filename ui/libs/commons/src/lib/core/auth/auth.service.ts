...
import { loadAuthSuccess } from '@rsa/global-data';
import Keycloak from 'keycloak-js';

import { from, Subject } from 'rxjs';
import { map } from 'rxjs/operators';
import { AuthEvent, AuthEventType } from './interfaces/auth-event';
import { AuthHasUserRole, AuthOptions, ExcludedUrl, ExcludedUrlRegex } from './interfaces/auth-options';
...
export class AuthService implements AuthHasUserRole {
   ...
   */
  private _keycloakEvents$: Subject<AuthEvent> = new Subject<AuthEvent>();
  /**
  ...
  constructor(private _httpClient: HttpClient, private _store: Store) {
    if (typeof dT_ !== 'undefined' && dT_.initAngularNg) {
      dT_.initAngularNg(_httpClient, Headers);
    }
  }
  ...
  private bindsKeycloakEvents(): void {
    ...
    this._instance.onAuthLogout = () => {
      this._keycloakEvents$.next({ type: AuthEventType.OnAuthLogout });
    };
    ...
    this._instance.onAuthSuccess = () => {
      this._keycloakEvents$.next({ type: AuthEventType.OnAuthSuccess });
    };
    ...
    this._instance.onActionUpdate = state => {
      this._keycloakEvents$.next({
        args: state,
        type: AuthEventType.OnActionUpdate,
      });
    };
   ...
   */
  public async init(options: AuthOptions = {}) {
    ...
    if (authenticated && this._loadUserProfileAtStartUp) {
      ...
      this._store.dispatch(
        loadAuthSuccess({
          auth: {
            user: { ...userProfile, id: this._instance.subject },
            id: this._instance.subject,
          },
        })
      );
    }
  ...
  public async login(options: Keycloak.KeycloakLoginOptions = {}) {
    ...
    if (this._loadUserProfileAtStartUp) {
      const userProfile = await this.loadUserProfile();
      this._store.dispatch(loadAuthSuccess({ auth: { user: { ...userProfile }, id: this._instance.subject } }));
    }
  }
  ...
  public async logout(redirectUri?: string) {
    ...
    this._store.dispatch(loadAuthSuccess({ auth: null }));
  }
   ...
   */
  getUserRoles(): string[] {
    return this._instance?.tokenParsed && this._instance.tokenParsed['groups']
      ? this._instance.tokenParsed['groups']
      : [];
  }

  hasUserRoles(roles: string[]): boolean {
   ...
   * described at {@link AuthEventType} enum and the function args from the keycloak-js
   * if provided any.
   *
   * @returns
   * A subject with the {@link AuthEvent} which describes the event type and attaches the
   * function args.
   */
  get keycloakEvents$(): Subject<AuthEvent> {
