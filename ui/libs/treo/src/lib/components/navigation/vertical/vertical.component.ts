...
import { TreoAnimations } from '../../../animations/index';
import { TreoScrollbarDirective } from '../../../directives/scrollbar/scrollbar.directive';
import { TreoMockApiService } from '../../../lib/mock-api/mock-api.service';
...
export class TreoVerticalNavigationComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  set treoScrollbarDirectives(treoScrollbarDirectives: QueryList<TreoScrollbarDirective>) {
    ...
    this._treoScrollbarDirectivesSubscription = merge(
      ...
      this.onCollapsableItemExpanded.pipe(
        tap(item => {
          if (item) {
            this.updateNavigationOnItemExpanded();
          }
        })
      )
    )
  ...
  set navigation(value: TreoNavigationItem[]) {
    if (this._defaultNavigation === undefined) {
      this.setDefaultNavigation(value);
    }
  ...
  setDefaultNavigation(value: TreoNavigationItem[]) {
    this._defaultNavigation = cloneDeep(value);
  }
  ...
  ngOnInit(): void {
    ...
    this._router.events
      ...
      .subscribe((e: RouterEvent) => {
        ...
        // Touchable - If navigating to Home Reset Navigation
        if (this.touchable && e.url.includes('home')) {
          this.resetHomeMenuStatus();
        }
      });
  }
  ...
  private updateNavigationOnItemExpanded() {
    // Touchable - If the nav menu is expanded keep this view after refresh
    if (this.touchable && this.onCollapsableItemExpanded.getValue()) {
      this.navigation[0].children = [];
      this.navigation = [...this.navigation];
      this.navigation[0].children.push(this.onCollapsableItemExpanded.getValue());
    }
  }
  ...
  resetHomeMenuStatus(): void {
    const treoNavItem = cloneDeep(this._defaultNavigation);
    this.navigation[0].children = [];
    this.navigation = treoNavItem;
    this.onCollapsableItemExpanded.next(null);
  }
